...
import { Send, Menu, Paperclip, Image as ImageIcon, Search, Zap, Loader2, Mic, MicOff, AlertCircle, Key, CheckCircle, X, Trash2 } from 'lucide-react';
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <button 
              onClick={handleSave}
              disabled={!key || saved}
              className={`flex-1 py-3 rounded-lg font-medium flex items-center justify-center gap-2 transition-all ${saved ? 'bg-green-500 text-white' : 'bg-brand-accent text-brand-900 hover:bg-blue-400'}`}
            >
              {saved ? <><CheckCircle size={18} /> Saved!</> : 'Save & Reload'}
            </button>
            <button 
               onClick={handleClear}
               className="px-4 py-3 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-colors"
               title="Remove Key"
            >
               <Trash2 size={18} />
            </button>
...
const Dashboard = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let errorMessage = `**System Error:** ${error.message || "Connection failed."}`;
      
      if (error.message === "MISSING_API_KEY" || error.message === "INVALID_API_KEY") {
        setShowApiKeyModal(true);
        errorMessage = `**Action Required:** Please enter a valid OpenAI API Key in settings to proceed.`;
      } 
      else if (error.message === "QUOTA_EXCEEDED") {
        setShowApiKeyModal(true);
        errorMessage = `**Billing Limit Reached:** Your OpenAI API Key has exceeded its quota or usage limit. \n\nPlease [check your billing details](https://platform.openai.com/account/billing) or enter a new, working key in Settings.`;
      }
